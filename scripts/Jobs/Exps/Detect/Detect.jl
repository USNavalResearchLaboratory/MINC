...
include("exps.jl")
##
function exp_detect(seed::Int, cfg_exp::NamedTuple; cfg=MINC.Config(),
    ...
    if bug_test
        cfg.epochs = 2
        exp_name = "Detect_warmup/set=$(cfg.set)_" * save_name_exp
    else
        exp_name = "Detect/set=$(cfg.set)_" * save_name_exp
    end
    ## Model AL1
    cfg.model_type = "A"
    cfg.A_L = 1
    cfg.savepath = "runs/$(exp_name)/AL1/seed=$(cfg.seed)/"
    MINC.detect(; cfg=cfg, dev_id=dev_id, bug_test=bug_test)
    ## Model Z
    cfg.model_type = "Z"
